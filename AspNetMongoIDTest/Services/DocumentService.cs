...
namespace AspNetMongoIDTest.Services
{
    public class DocumentService
    {
        ...
        public async Task NewCollection(string databaseName, string value)
        {
            var db = _client.GetDatabase(databaseName);
            await db.CreateCollectionAsync(value);
        }

        public async Task DeleteCollection(string databaseName, string value)
        {
            var db = _client.GetDatabase(databaseName);
            await db.DropCollectionAsync(value);
        }
        public async Task NewDatabase(string value)
        {
            var db = _client.GetDatabase(value);
            await db.CreateCollectionAsync("Demo");
        }
        public async Task DeleteDatabase(string value)
        {
            await _client.DropDatabaseAsync(value);
        }
    }
}
